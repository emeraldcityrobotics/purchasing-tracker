...
export class RequestComponent {
  ...
  onDepartmentQueryChange() {
    const match = this.departments().find(department => department.name === this.departmentQuery);
    this.departmentId = match ? match.id.toString() : '';
  }

  onVendorQueryChange() {
    const match = this.vendors().find(vendor => vendor.name === this.vendorQuery);
    this.vendorId = match ? match.id.toString() : '';
  }
  ...
  submit() {
    this.error.set(''); if (!this.requesterName || !this.departmentId || !this.vendorId || this.items.some(item => !item.product_name || item.quantity < 1 || item.unit_price < 0)) {
      ...
      this.notifications.success('Purchase request submitted'); this.orderName = ''; this.items = [this.newItem()]; this.notes = '';
